...
| 운영 리포트 | 일간/주간/스케일링/RiskOps 리포트, 포함 RCA 참조, 전처리된 metric summary, 이메일 발송 큐 연결을 관리합니다. |
...
- LLM run은 Farmer Chatbot, Admin Copilot, RCA, 운영 리포트가 공통으로 사용하는 실행 감사 단위이며, prompt version, job, session, masked input/output과 연결됩니다.
...
- Agent snapshot은 `observability_snapshots`를 공통 저장소로 사용하며, `session_public_id`, `llm_run_public_id`, `snapshot_payload`로 Farmer/Admin/RCA/리포트 실행에서 LLM run과 MCP tool call 근거를 추적합니다.
- 운영 리포트는 incident 요약, 포함된 RCA report, 전처리된 metric summary, 예측 오차 요약, KEDA/HPA scaling event 요약을 포함할 수 있습니다.
...
- 운영 리포트 이메일 발송은 1차 MVP에서 `notification_outbox`를 사용합니다. `channel=EMAIL`, `recipient`, `payload.report_id`, `payload.subject`, `payload.html_body`로 SMTP 발송 요청과 `SENT`/`FAILED` 결과를 추적합니다.
- Alertmanager 기반 RCA 알림도 별도 테이블을 추가하지 않고 `notification_outbox`를 재사용합니다. 신규 firing 직후 preliminary email은 `related_table=incidents`, 최종 RCA email은 `related_table=rca_reports`로 연결하고, `payload.notification_stage=preliminary|final` 및 `idempotency_key`로 발송 단계와 중복 방지를 추적합니다.
- 알림과 승인 record는 운영, 리스크, 리포트, RCA, MCP Tool 호출 워크플로우에 연결됩니다.
- Approval request와 notification outbox는 초기 구현에서 목록 조회 API를 먼저 제공하고, 승인/반려/재시도 실행은 후속 worker와 Human-in-the-loop 고도화에서 연결합니다.

## 운영 리포트 MVP ERD 정책

일간/주간 운영 리포트는 기존 `ai.ops_reports`를 중심으로 저장합니다.

| 테이블 | 목적 |
|---|---|
| `ai.ops_reports` | `DAILY`, `WEEKLY` 운영 리포트의 기간, 제목, LLM 요약, section JSON, metric JSON, 생성 상태를 저장합니다. |
| `ai.report_incidents` | 운영 리포트에 포함된 incident 목록과 incident별 요약을 저장합니다. |
| `ai.ops_report_rca_refs` | 운영 리포트에 실제 포함된 `rca_reports`를 명시적으로 연결합니다. 같은 incident에 여러 RCA가 있거나 리포트 생성 이후 RCA가 추가되어도 리포트 포함 이력을 재현할 수 있게 합니다. |
| `ai.report_metric_summaries` | 운영 리포트에 포함할 Prometheus/Loki/DB/Prediction/KEDA/HPA metric 요약을 저장합니다. |
| `ai.notification_outbox` | 운영 리포트 이메일 발송 요청, SMTP 발송 결과, 실패 사유를 저장합니다. |

`ai.ops_report_rca_refs`는 다음 관계를 가집니다.

| 컬럼 | 설명 |
|---|---|
| `report_public_id` | `ai.ops_reports.public_id`를 참조합니다. |
| `rca_report_public_id` | `ai.rca_reports.public_id`를 참조합니다. |
| `incident_public_id` | 조회 편의를 위해 RCA가 속한 `ai.incidents.public_id`를 함께 저장합니다. |
| `included_reason` | 기간 조건, severity 조건 등 RCA가 리포트에 포함된 이유를 저장합니다. |
| `created_at` | 연결 생성 시각입니다. |

`ai.report_metric_summaries.source_type`은 운영 리포트 MVP에서 기존 observability source 외에 `PREDICTION`, `PREDICTION_ERROR`, `KEDA`, `HPA`, `SCALING` source를 지원해야 합니다.

PDF 또는 첨부 파일 발송 이력이 필요해지는 경우에는 후속 단계에서 `ops_report_exports` 또는 `ops_report_deliveries`를 추가합니다. 1차 MVP에서는 HTML 이메일 본문, SMTP 발송, `notification_outbox` 상태 추적만 사용합니다.
