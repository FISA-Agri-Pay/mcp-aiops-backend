...
| InfraOps와 RCA | incident, alert payload, 관측 snapshot, RCA 보고서, RCA 피드백을 관리합니다. |
...
| 농업 조언 | 농업 조언 case와 생성된 조언 결과를 관리합니다. |
| Admin RiskOps | 리스크 분석 보고서, 분석 대상, 재해 리스크 시뮬레이션 결과를 관리합니다. |
| 예측과 스케일링 | 모델 버전, 예측 실행, 예측 metric, 실제 metric, 예측 오차, scaling event를 관리합니다. |
...
- 농업 조언 case는 하나 이상의 조언 결과를 생성할 수 있습니다.
- 리스크 분석 보고서는 여러 비즈니스 참조를 분석할 수 있고, 재해 리스크 시뮬레이션과 연결될 수 있습니다.
- 예측 실행은 예측 metric을 생성하고, 실제 metric과 비교되어 예측 오차 metric으로 저장됩니다.
- Scaling event는 KEDA/HPA 분석을 위해 수집되며 예측 metric과 연결될 수 있습니다.
