...
- 운영 리포트 이메일 발송은 1차 MVP에서 `notification_outbox`를 사용합니다. `channel=EMAIL`, `recipient`, `payload.report_id`, `payload.subject`, `payload.html_body`로 발송 요청과 상태를 추적합니다.
...
| `ai.notification_outbox` | 운영 리포트 이메일 발송 요청과 발송 상태를 저장합니다. |
...
PDF 또는 첨부 파일 발송 이력이 필요해지는 경우에는 후속 단계에서 `ops_report_exports` 또는 `ops_report_deliveries`를 추가합니다. 1차 MVP에서는 HTML 이메일 본문과 `notification_outbox`만 사용합니다.
